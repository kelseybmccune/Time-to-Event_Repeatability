...
We simulated time-to-event data with a single predictor variable (sex) and a single random effect (i.e. cluster or individual) $\alpha$. The data were generated following models in equation (3) and (4) in the main text. We considered three different values for the random effect variance $\sigma^2_{\alpha}=1, 2, 3$ (frailty variance), two levels of censoring $c= 0, 0.15$ and two time interval splitting numbers $int\_n = (2, 4)$. We set the true value of the predictor beta parameter to be $0.5$. We considered $n=1000$ observations and $n_{cluster}=100$ the number of clusters to be $n_{cluster}=10$ (i.e. individuals).

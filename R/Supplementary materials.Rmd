...
coxme_icc_ci(ctw.cox, upper.multiplier = 10) # ICC = 0.33 (0.22 - 0.48)
coxme_pval(ctw.cox,ctw,boot=100) # Likelihood ratio test and bootstrapping indicates a very small p-value (p = 0).
...
var <- VarCorr(dist.cox)$ID[[1]] # 0.64
var/(var + pi^2/6) # ICC for ID = 0.28
var2 <- VarCorr(dist.cox)$SPP[[1]] # 0.37
var2/(var2 + pi^2/6) # ICC for SPP = 0.18
...
var <- VarCorr(dist.cox.int)$ID[[1]] # 0.64
var/(var + pi^2/6) # ICC for ID = 0.28
var2 <- VarCorr(dist.cox.int)$SPP[[1]] # 0.37
var2/(var2 + pi^2/6) # ICC for SPP = 0.18
